  ...
  return (
    <section className="mt-6">
      <div className="mb-2.5 flex items-center gap-2 px-1">
        ...
        <span className="text-xs font-medium text-slate-400">{todos.length}</span>
      </div>
...
function ActionBanner({
  ...
  return (
    <button
      onClick={onClick}
      className="mt-4 flex w-full items-center gap-3 rounded-2xl border border-brand-100 bg-brand-50 p-4 text-left transition active:scale-[0.99]"
    >
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-600 text-white">
        <Icon className="h-5 w-5" />
      </div>
      <p className="flex-1 text-sm font-semibold text-brand-800">{text}</p>
