      ...
      <div className="flex flex-col gap-4">
        ...
        {isFetching && !data ? (
          ...
          <div className="flex flex-col gap-3">
            {rows.map((row) => {
              const buzzing = buzz.isPending && buzz.variables === row.todo
              return (
                <div key={row.todo} className={card}>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                    ...
                    {row.assignee_email ? (
                      <a href={`mailto:${row.assignee_email}`} className="inline-flex items-center gap-1 text-brand-600 dark:text-brand-300">
                        <Mail className="h-3.5 w-3.5" /> {row.assignee_email}
                      </a>
                    ) : null}
                    {row.assignee_mobile ? (
                      <a href={`tel:${row.assignee_mobile}`} className="inline-flex items-center gap-1 text-brand-600 dark:text-brand-300">
                        <Phone className="h-3.5 w-3.5" /> {row.assignee_mobile}
                      </a>
                    ) : null}
                  </div>
                  <button
                    onClick={() => onBuzz(row.todo, row.assignee_name)}
                    disabled={buzzing}
