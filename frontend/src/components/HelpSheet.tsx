...
import type { HelpEntry } from '@/lib/types'

// (i) explanations for one feature. Opens on the term the reader tapped and lists the
// rest below it, so someone who tapped one question can keep reading without hunting.
// `entries` is the feature's help map — one sheet serves every feature that has one.
export function HelpSheet({
  entries, term, onClose,
}: { entries: HelpEntry[]; term: string | null; onClose: () => void }) {
  useEffect(() => {
    if (!term) return
  ...
  if (!term) return null
  const focused = entries.find((h) => h.term === term)
  const rest = entries.filter((h) => h.term !== term)
