...
// Web twin of BulkAddSheet: shared fields once, per-row title/notes/blocked_by.
// Recurrence is deliberately not offered — use single-add for a recurring task.
export function BulkAddDialog({ open, onClose, projectDetail, team, defaultGroup, siblings = [], onCreated }: Props) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="text-xs font-semibold uppercase tracking-wide text-muted">Shared by every task</div>
        ...
        {prog && create.isPending && (
          <div>
            <div className="mb-1 text-sm font-semibold text-muted">Creating {prog.done} / {prog.total}</div>
        ...
        <div className="text-xs font-semibold uppercase tracking-wide text-muted">Tasks ({filled.length})</div>
