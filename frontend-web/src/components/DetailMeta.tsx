...
export function DetailMeta({ d }: { d: ProjectDetail }) {
  ...
  const hasPricing = (d.bonus_amount != null && d.bonus_amount > 0) || (d.discount != null && d.discount > 0)
      ...
      {hasPricing && (
        <Section title="Reward">
          <div className="flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
            {d.bonus_amount != null && d.bonus_amount > 0 && (
              <div className="flex justify-between">
                <span>Bonus</span>
                <span className="font-medium">
                  {d.reward_type === 'Point'
                    ? `${d.bonus_amount.toLocaleString('id-ID')} pts`
                    : `Rp ${d.bonus_amount.toLocaleString('id-ID')}`}
                </span>
              </div>
            )}
