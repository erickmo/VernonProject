  ...
  const cols: Column<LastSeenRow>[] = [
    {
      ...
      render: (u) => (
        <div className="flex items-center gap-3">
          <PresenceAvatar name={u.full_name || u.name} image={u.user_image} size={32} online={presenceOf(u.last_active, win).online} />
            ...
            <div className="flex flex-wrap gap-3">
              {active.map((u) => (
                <div key={u.name} className="flex w-16 flex-col items-center gap-1" title={u.full_name || u.name}>
                  <PresenceAvatar name={u.full_name || u.name} image={u.user_image} size={44} online />
